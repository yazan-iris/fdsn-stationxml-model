...
public class StationXmlIterator implements Iterator<Station>, AutoCloseable {
	...
	private DateTimeFormatter format = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss[z]")
			.withZone(ZoneId.of("UTC"));
	
	public StationXmlIterator(InputStream inputStream) throws IOException {
		this.inputStream = inputStream;
		XMLInputFactory xmlFactory = XMLInputFactory.newInstance();
		try {
			this.xmlEventReader = xmlFactory.createXMLEventReader(inputStream);

			JAXBContext jc = JAXBContext.newInstance(edu.iris.station.model.ObjectFactory.class);
				...
				if ("Network".equals(startElement.getName().getLocalPart())) {
					...
					while (attributes.hasNext()) {
						...
						if (attributeName.equals("code")) {
							...
							if (startString != null) {
								network.setStartDate(ZonedDateTime.parse(startString, format));
							}
						} else if (attributeName.equals("endDate")) {
							String endString = attribute.getValue();
							if (endString != null) {
								network.setEndDate(ZonedDateTime.parse(endString, format));
							}
						} else {
							// Do nothing for now
						}
